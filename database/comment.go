...
func GetReviewCommentById(db *sql.DB, id int) Comment {
	sql_statement := `
	SELECT c.content, u.id, u.username, c.is_deleted, c.timestamp, c.likes, c.dislikes,
		(
			SELECT ARRAY(
			 	SELECT DISTINCT ch.id
				FROM Comments_review ch
				WHERE ch.parentid = c.id
			)
		) as childComments
	FROM Comments_review c JOIN Users u ON c.authorid = u.id
	WHERE c.id = $1
	`
	rows, err := db.Query(sql_statement, id)
	if err != nil {
...
func DeleteReviewCommentById(db *sql.DB, ReviewcommentId int, userId int) error {
	if !isAuthorizedReview(db, ReviewcommentId, userId) {
		return errors.New("Not authorized")
	}

	sql_statement := `
	UPDATE Comments_review
	SET is_deleted = TRUE
	WHERE id = $1
	`
	_, err := db.Exec(sql_statement, ReviewcommentId)

	return err
}

func isAuthorizedReview(db *sql.DB, reviewcommentId int, userId int) bool {
	var status bool
	sql_statement := `
	SELECT 1 FROM
	Comments_review c JOIN Users u
	ON c.authorid = u.id
	WHERE c.id = $1 AND u.id = $2
	`
	rows, err := db.Query(sql_statement, reviewcommentId, userId)
	if err != nil {
		panic(err)
	}
	defer rows.Close()
...
func getNumberOfCommentsByReview(db *sql.DB, moduleid int) int {
	sql_statement := `
	SELECT COUNT(*)
	FROM Comments_review c
	WHERE c.moduleid = (select r.moduleid from reviews r)
	`
